...
-- 주의 : 혼자서 COMMIT, ROLLBACK 수행 금지!
-- 윈도우 > 설정 > 연결 > 연결유형 Auto commit by default 체크 해제되어있는지 다시 확인!
...
-- 선택 안된 컬럼은 값이 NULL 들어감(DEFAULT 존재 시 DEFAULT 설정한 값 삽입)

INSERT INTO EMPLOYEE2 (EMP_ID, EMP_NAME, EMP_NO, EMAIL, PHONE, 
											DEPT_CODE, JOB_CODE, SAL_LEVEL, SALARY)
VALUES('900', '홍길동', '991215-1234567', 'gonh_gd@or.kr',
...
-- * 여러 컬럼을 한번에 수정할 시 콤마(,)로 컬럼 구분
-- D9 / 총무부 --> D0 / 전략기획팀 변경

UPDATE DEPARTMENT2 d 
SET DEPT_ID = 'D0',
DEPT_TITLE = '전략기획팀' --WHERE 절 직전 컬럼 콤마 작성 X
WHERE DEPT_ID = 'D9'
AND DEPT_TITLE = '총무부';
...
INSERT INTO EMP_M02
VALUES (999, '곽두원', '561016-1234567', 'kwack@or.kr',
...
-- DELETE FROM 테이블명
-- [WHERE 조건 설정];
-- 만약에 WHERE 조건을 설정하지 않으면 모든 행 삭제!
