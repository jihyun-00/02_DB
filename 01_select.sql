...
-- ''는 문자열을 의미
-- NUMBER 타입인 1(숫자)과 문자열인 '1'이 같다고 인식 중
-- DUAL : ORACLE에서 사용하는 더미테이블
-- 더미테이블? 실제 데이터를 저장하는 게 아닌, 임시 계산이나 테스트 목적 사용
...
-- EMPLOYEE 테이블에서 이메일의 _앞의 글자가 3글자인 사원만
-- 이름, 이메일 조회
SELECT * FROM EMPLOYEE;
...
--EMPLOYEE 테이블에서
--이메일 _앞에 4글자이면서
--부서코드가 'D9' 또는 'D6'이고 --> AND가 OR보다 우선순위가 높다 ex) () 사용가능
...
-- EMPLOYEE 테이블에서 보너가 없는 사원의 이름, 보너스 조회
SELECT EMP_NAME, BONUS
FROM EMPLOYEE
WHERE BONUS IS NULL;
