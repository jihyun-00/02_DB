...
-- *** DEPT_COPY의 CEPT_TITLE 컬럼에 NOT NULL 제약조건 추가 / 삭제 ***
ALTER TABLE DEPT_COPY
...
ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE NOT NULL; --CEPT_TITLE컬럼에 NOT NULL 적용
...
--> VARCHAR2(3)로 변경해보기 (남는 바이트 메모리 반환)

-- DEPT_ID 컬렁 데이터 타입 수정
ALTER TABLE DEPT_COPY
...
-- 컬럼 삭제 시 유의사항!
-- 데이블이란? 행과 열로 이루어진 DB의 가장 기본적인 객체
--> 테이블 최소 1개 이상의 컬럼이 존재하기 때문에
-- 모든 컬럼을 다 삭제할 순 없다.
...
-- DEPT_COPY 테이블에 PIMARY KEY 추가(컬럼 : DEPT_ID, 제약조건명 : D_COPY_PK)
...
-- DROP TABLE 테이블명 [CASCADE CONSTRAITNS]; --S 붙음
-- CASCADE CONSTRAITNS : 제약조건이 달려있는 것 모두 삭제
...
CREATE TABLE TB2(
	...
	TB2_COL NUMBER REFERENCES TB1 --FK 제약조건 설정(부모 테이블 PK 참조)
); --자식 테이블
...
--> DDL이 종료된 후 DML 구문 수행할 수 있도록 권장!
...
ROLLBACK;
-- DDL 구문 중 ALTER를 사용해서
-- 그 시점에 강제 COMMIT이 되었다
-- 강제 COMMIT이 이루어져서 ROLLBACK 불가
