...
-- 단일행 함수 : N개의 값을 읽어서 연산 후 N개의 결과 반환

-- 그룹 합수 : N개의 값을 읽어서 연산 후 1개의 결과 간환(합계, 평규느 최대, 최소)

-- 함수는 SELECT 문의
-- SELECT, WHERE절, ORDER BT,절, ORDER BY절, HAVING 절사용 가능
...
-- 문자열을 5번째 문자부터 검색하여 두번쨰 B 위치 조회
SELECT INSTR('AABAACAABBAA', 'B', 5, 2) FROM DUAL; -- 10

-- EMPLOOYEE 테이블에서 사원명, 이메일, 이메일 중 '@' 위치 조회
...
-- 주어진 컬럼이나 문자열의 앞, 뒤, 양쪽에 지정된 문자 제거
-- -> 양쪽 공백 제거에 많이 사용함
...
SELECT SYSDATE FROM DUAL; --2025-03-10 12:11:15.000

--SYSTIMESTAMP : SYSDATE + MS 단위 추가
SELECT SYSTIMESTAMP FROM DUAL; --2025-03-10 12:12:14.716 +0900
...
-- 숫자 -> 문자 변환 시 포맷 패턴
-- 9 : 숫자 한칸 의미, 여러개 작성 시 오른쪽 정렬
-- 0 : 숫자 한칸 의미, 여러개 작성 시 오른쪽 정렬 + 빈칸 0 추가
-- L : 현재 DB에 설정된 나라의 화폐 기호

SELECT TO_CHAR(1324, '99999') FROM DUAL;
SELECT TO_CHAR(1324, '00000') FROM DUAL;
...
SELECT EMP_NAME, NVL2(BONUS, 'O', 'X') 보너스수령
FROM EMPLOYEE;
...
-- 급여 최댓값, 가장 최근 입사일, 알파벳 순서가 가장 느림 이메일 조회
...
FROM EMPLOYEE
WHERE SALARY = (SELECT MAX(SALARY) FROM EMEMPLOYEE);

-- 서브쿼리 + 그룹함수
SELECT MAX(SALARY) FROM EMEMPLOYEE;


-- COUNT() : 행 개수를 헤어려서 반환
-- COUNT(컬럼명) : NULL을 제외한 실제값이 기록된 행 개수를 리턴
-- COUNT(*) : NULL을 포함한 전제 행 개수 리턴
-- COUNT([DISTINCT] 컬럼명) : 중복 제거한 행 개수 리턴
